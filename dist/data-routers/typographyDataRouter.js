...
export const typographyDataRouter = (block) => {
    ...
    const { context: { parent } = {}, attrs: { fontSize, style: { spacing } = {}, level } = {}, } = block;
    let defaultClasses = [];
    ...
    if (fontSize) {
        ...
        defaultClasses.push(`xs:text-${fontSize}`, `sm:text-${fontSize}`, `md:text-${fontSize}`, `lg:text-${fontSize}`, `xl:text-${fontSize}`, `2xl:text-${fontSize}`);
    }
    ...
    const hasNoBottomMargin = (!spacing?.margin?.bottom || spacing?.margin?.bottom == "0") &&
        !classes.includes("mb-");
    if (block.name == "core/heading" && level == "2" && hasNoTopMargin) {
        defaultClasses.push("mt-8 first:mt-0");
    }
    return {
        className: cx(defaultClasses, classes, parent && hasNoTopMargin && "mt-0", parent && hasNoBottomMargin && "mb-0"),
        style: styles,
